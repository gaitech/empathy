...
get_header(); ?>
<div class="site_base_url" style="display:none" ><?php echo get_site_url(); ?></div>
    ...
    <section class="countries_real__stories">
      ...
      foreach ($query->posts as $key => $value) {
        ...
        <div class="more_story">
          ...
          $query = new WP_Query();
          $query->query($args);
          $total_result = count($query->posts);
          if($total_result  > 3) {
            echo '<div class="total_result_count" style="display:none"></div>';
          } else {
            echo '<div class="total_result_count" style="display:none">done</div>';
          }
          ?>

          <?php
          if($_REQUEST['country']) {
            echo do_shortcode('[ajax_load_more container_type="div" post_type="story" posts_per_page="3" meta_key="country" meta_value="'.$_REQUEST['country'].'" meta_compare="LIKE" offset="3" pause="true" scroll="false" button_label="MORE STORIES"]');
          } else {
            echo do_shortcode('[ajax_load_more container_type="div" post_type="story" posts_per_page="3" offset="3" pause="true" scroll="false" button_label="MORE STORIES"]'); }?>
        </div>
    </section>
  </div>
